...
#db.execute("INSERT INTO users (username, hash) VALUES (:username, :hash)", username="rootme", hash=generate_password_hash("rootme"))
...
def index():
    ...
    for i in range(len(stocks)):
        names.append(lookup(stocks[i]["symbol"])["name"])
...
def buy():
    ...
    if request.method == "POST":
        #Ensure user have enought money
...
def register():
    ...
    if request.method == "POST":
        ...
        #Ensure password matchs the confirmation password
...
def sell():
    ...
    if request.method == "GET":
        ...
        if quantity_owned < int(request.form.get("shares")):
            ...
            db.execute("UPDATE stocks SET sale_date=datetime('now') WHERE user_id=:user_id AND id=:id", user_id=session["user_id"], id=request.form.get("symbol"))
